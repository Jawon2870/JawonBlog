...
// 定义博主QQ邮箱，博主的评论会显示特殊标志
var masterQQEmail = '2870881842@qq.com';
		...
		*/
		...
		o.prototype.Q = function (e) {
			...
			var C = function (t, n, r) {
				...
				// 判断评论是博主的、还是好友的、还是访客的
				// var ism = e.config.master.includes((0, s.default)(t.get("mail"))),
				// 	isf = e.config.friends.includes((0, s.default)(t.get("mail").toLowerCase())),
				var ism = false, isf = false;
				if(t.get("mail") == masterQQEmail) ism = true;

				var	gat = ism ? '<span class="vtag vmaster">' + e.config.tagMeta[0] + '</span>' : isf ? '<span class="vtag vfriend">' + e.config.tagMeta[1] + '</span>' : '<span class="vtag vvisitor">' + e.config.tagMeta[2] + '</span>';
			...
			var z = e.$el.find(".vsubmit"),
				L = function (t) {
					// 添加检测昵称和QQ邮箱填写是否合法的功能
					let inputNick = document.querySelector(".vnick.vinput");
					let inputMail = document.querySelector(".vmail.vinput");
					// 如果昵称为空就默认昵称
					if (inputNick.value == '') {
						alert("请输入您的昵称！");
						return;
...
function setInput() {
	...
	if (!inputNick || !inputMail) return false;
	...
	inputMail.addEventListener("focus", function() {
		...
		if (!regex.test(this.value)) {
			// 如果不包含，就在输入内容后添加 @qq.com
			this.value = this.value.concat("@qq.com");
		}
	});
	return true;
}
...
if(setInput() == false){
	let loop1 = setInterval(() => {
		if (setInput()) clearInterval(loop1);
	}, 500);
}
//添加完毕